...
const UserInput = () => {
    ...
    const onImageAnalyzeClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (fileInput?.files && fileInput.files[0]) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    const ingredientLabels = data.ingredientLabels.map((label: { name: string }) => label.name);
            ...
            <div className="flex sm:flex-row flex-col sm:gap-10 gap-2 h-full">
                <div className="flex flex-col sm:gap-5 gap-2 sm:w-1/2 sm:h-full h-[60%]">
                    <div>
                        ...
                        <Input ref={fileInputRef} accept="image/*" id="picture" type="file" className="border-black" onChange={handleImageChange} />
                    </div>
                    <div className="rounded-xl border-2 h-full border-dotted border-black/20 sm:p-4 p-1 flex flex-col gap-2 justify-center items-center text-5xl">
                        {selectedImage ? (
                            <img src={selectedImage} alt="Selected" className="w-auto sm:max-h-[350px] max-h-[190px] object-contain rounded-xl" />
                        ) : (
                            <CiImageOn />)}
                    </div>
                    <Button className="w-full" onClick={onImageAnalyzeClick}>Recognize</Button>
                ...
                <div className="flex flex-col gap-2 sm:w-1/2 sm:h-full h-[40%]">
                    <div className="h-full w-full flex flex-col gap-1">
                        ...
                        {loadingBadges ? (
                            ...
                            <div id="ingredients" className=" bg-orange-200 rounded-xl p-4 overflow-auto flex flex-wrap gap-2">
                                    ...
                                    {ingredients.length > 0 ? (
                                        ingredients.map((ingredient) => (
                                            <IngredientBadge
                                                ...
                                                onRemove={() => setIngredients(ingredients.filter((i) => i !== ingredient))}
                                            />
                                        ))
                                    ) : (
                                        <p className="text-gray-700 font-custom">No ingredients recognized. Try uploading your image of ingredients</p>
                                    )}
                                </div>
                            </div>
                        )}
                    ...
                    <div id="cooking-time" className="flex flex-col">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button className="bg-orange-400 text-white">{cookingTime}</Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent className="w-56">
                                <DropdownMenuRadioGroup value={cookingTime} onValueChange={setCookingTime}>
                                    <DropdownMenuRadioItem value="30min">Less than 30 minutes</DropdownMenuRadioItem>
                                    <DropdownMenuRadioItem value="1hour">Less than 1 hour</DropdownMenuRadioItem>
                                    <DropdownMenuRadioItem value="2hours">Less than 2 hours</DropdownMenuRadioItem>
